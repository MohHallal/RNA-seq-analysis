...
gg2 <- ggplot(data, aes(x = PC3, y = PC4, label = name)) +
  ...
  theme(legend.position = "top",
        ...
        legend.title = element_text(size = 18),
        plot.background = element_rect(fill = "white"))
ggsave("PCA_expression_salmon/PC1_PC2_salmon_expression.png", gg1, width = 10, height = 8, units = "in")
ggsave("PCA_expression_salmon/PC3_PC4_salmon_expression.png", gg2, width = 10, height = 8, units = "in")

sink("PCA_expression_salmon/pca_summary.txt")
print(data)
